...
LLAMADO POR			:
DESCRIPCION			:	Obtiene el registro de la institución por codigo modular
REVISIONES			:
-----------------------------------------------------------------------------------------------------------
...
--  TEST:			
/*
	EXEC USP_MAESTROS_SEL_INSTITUCION '1468131','mruiz'
*/
**********************************************************************************************************/
CREATE PROCEDURE [dbo].[USP_MAESTROS_SEL_INSTITUCION] 
(
	   @CODIGO_MODULAR_IE	Varchar(9),
...
BEGIN --> #temp001 reemplaza a una vista antigua que obtenia datos de maestro.institucion
	...
	SELECT
		...
		--CASE WHEN v.TIPO_GESTION = 10061 THEN 3 WHEN v.TIPO_GESTION = 10062 THEN 4 END AS IdTipoGestionIe,		--FRAMOS_CAMBIO_PROVISIONAL_VISTA
...
END 
	...
	WHERE 
		B.CODIGO_MODULAR=@CODIGO_MODULAR_IE
GO
