...
LLAMADO POR			:
DESCRIPCION			:	Lista los registros realizados a traves de la carga masiva actas
REVISIONES			:
-----------------------------------------------------------------------------------------------------------
...
--  TEST:	
	EXEC USP_EVALUACION_SEL_CARGA_ACTAS_PAGINADO 2945,'','',0,'',0,0,0
**********************************************************************************************************/
CREATE PROCEDURE [dbo].[USP_EVALUACION_SEL_CARGA_ACTAS_PAGINADO](
--DECLARE
	 @ID_INSTITUCION INT,
...
SET NOCOUNT ON;
	...
	(
		...
		SELECT 	
			...
			cma.ID_CARGA_MASIVA_ACTA IdCargaMasivaActa,
			...
			Total = COUNT(1) OVER ( ) 
		FROM transaccional.carga_masiva_actas cma
			INNER JOIN transaccional.carga_masiva_nominas cmn ON cmn.ID_CARGA_MASIVA_NOMINA = cma.ID_CARGA_MASIVA_NOMINA
		...
		WHERE cmn.ES_ACTIVO=1 AND cma.ES_ACTIVO = 1 AND cmn.ID_INSTITUCION = @ID_INSTITUCION AND
