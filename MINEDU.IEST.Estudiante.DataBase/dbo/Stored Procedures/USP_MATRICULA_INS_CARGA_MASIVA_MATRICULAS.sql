/*--
USP_MATRICULA_INS_CARGA_MASIVA_MATRICULAS 71, 442, 4, 7, 101, 112, 773, 106, 6, 'D.N.I.', '73064775', 'CLEMENTE', 'OTEO', 'ANGY LISETTH', 'FEMENINO', 
'1992-02-15', 'NO', '', '4379|4381|4383|4384', '110907', '140141', 1, 92330, 92330, 'AV. AVIACIÓN 258 URB. LA AVENIDA',
1, 7,'ERROR EN REGISTRO', 'MALVA' 

USP_MATRICULA_INS_CARGA_MASIVA_MATRICULAS 70, 442, 4, 7, 101, 112, 773, 106, 6, 'D.N.I.', '44492259', 'ZAPATA', 'CASTRO', 'GABRIELA', 'FEMENINO', 
'1992-02-15', 'SI', 'MOTORA', '4379|4381|4382|4383', '110907', '140141', 3, 92330, 92330, 'AV. LOS CONDORES 147 URB. LOS MOLINOS',
1, 0,'', 'MALVA' 
 */
CREATE PROCEDURE [dbo].[USP_MATRICULA_INS_CARGA_MASIVA_MATRICULAS]
(
	@ID_CARGA INT,
	...
	@ASIGNACIONES_UNIDADES_DIDACTICAS NVARCHAR(MAX), --'4379|4380|4381|4383|4384' --UD MATRICULADAS
...
BEGIN
	...
	BEGIN TRY
			...
			SET @ID_PERIODO_LECTIVO_INSTITUCION = (SELECT ID_PERIODOS_LECTIVOS_POR_INSTITUCION FROM transaccional.periodo_academico WHERE ID_PERIODO_ACADEMICO= @ID_PERIODO_ACADEMICO)
			...
			EXEC @ID_DET_ARCHIVO = USP_ARCHIVO_INS_CARGA_DETALLE	@ID_CARGA,
														...
														@FECHA_NACIMIENTO, @ES_DISCAPACITADO, @ID_TIPO_DISCAPACIDAD, @ASIGNACIONES_UNIDADES_DIDACTICAS, @UBIGEO_NACIMIENTO, 
			...
			BEGIN
				...
				CREATE TABLE #temp (
					...
					ProgramaEstudios		VARCHAR(150),
					IdPlanEstudios			INT,
					IdTipoItinerario		INT,
					PlanEstudios			VARCHAR(30),
					TurnoInstitucion		VARCHAR(30),
					Condicion				VARCHAR(30),
					EsMatriculaPermitida	INT,
					IdSemestreAcademico		INT,
					Semestre				VARCHAR(10),
					Matriculado				INT,
					IdLiberacionEstudiante	INT,
					IdTrasladoEstudiante	INT
				);
				
				INSERT INTO #temp		
				EXEC USP_MATRICULA_SEL_ESTUDIANTE_SITUACION @ID_PERIODO_LECTIVO_INSTITUCION, @ID_TIPO_DOCUMENTO, @NUMERO_DOCUMENTO;

				DECLARE @EsMatriculaPermitida INT, @Condicion varchar(30)
			
				SELECT @EsMatriculaPermitida = EsMatriculaPermitida, @Condicion= Condicion FROM #temp
				DROP TABLE #temp

				IF @EsMatriculaPermitida = 0
				SET @MENSAJES='La matrícula no está permitida. Actualmente el estudiante tiene ' + @Condicion
			...
			END		
				...
				--SET @ID_MATRICULA_ESTUDIANTE = (SELECT ID_MATRICULA_ESTUDIANTE FROM  transaccional.matricula_carga_masiva 
				...
				IF @ID_MATRICULA_ESTUDIANTE IS NULL
					BEGIN
					...
					(	
						...
						SET @ID_MATRICULA_ESTUDIANTE= CONVERT(INT,@@IDENTITY)
					END	
					ELSE
					BEGIN
						UPDATE transaccional.matricula_estudiante
						SET ID_PROGRAMACION_MATRICULA = @ID_PROGRAMACION_MATRICULA,
					...
					END						
				EXEC USP_MATRICULA_INS_CARGA_MASIVA_MATRICULAS_ASIGNACIONES @ID_MATRICULA_ESTUDIANTE, 
