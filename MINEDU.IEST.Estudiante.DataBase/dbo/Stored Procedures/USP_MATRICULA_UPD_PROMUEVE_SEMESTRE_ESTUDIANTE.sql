...
1.1			03/05/2022		JCHAVEZ			VALIDAR ULTIMO SEMESTRE SEGÚN NIVEL FORMATIVO PARA NO PROMOVER A UN SEMESTRE QUE NO CORRESPONDE

TEST:  
	USP_MATRICULA_UPD_PROMUEVE_SEMESTRE_ESTUDIANTE 306, 1235, 101,1135,3
**********************************************************************************************************/
CREATE PROCEDURE [dbo].[USP_MATRICULA_UPD_PROMUEVE_SEMESTRE_ESTUDIANTE]
(
	@ID_INSTITUCION					INT,
	...
	BEGIN TRY
		...
		BEGIN TRAN TransactSQL
			...
			Select
					...
					mc.ID_CARRERA,mc.TIPO_NIVEL_FORMACION,
					Estado = 1	
		...
		UPDATE E
		SET IdSemestreAcademico = E_SEM_N.ID_ENUMERADO,
			SemestreAcademico = E_SEM_N.VALOR_ENUMERADO
		FROM #tmp_Estudiantes E
		INNER JOIN sistema.enumerado E_SEM ON E.IdSemestreAcademico = E_SEM.ID_ENUMERADO
		INNER JOIN sistema.enumerado E_SEM_N ON E_SEM.ID_TIPO_ENUMERADO = E_SEM_N.ID_TIPO_ENUMERADO AND E_SEM_N.ORDEN_ENUMERADO = E_SEM.ORDEN_ENUMERADO + 1
		INNER JOIN maestro.nivel_formacion MNF ON MNF.CODIGO_TIPO = E.TIPO_NIVEL_FORMACION 
		WHERE E.IdSemestreAcademico < (CASE WHEN MNF.SEMESTRES_ACADEMICOS = 8 THEN 138 
